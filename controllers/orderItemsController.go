...
func GetOrderItemsByOrder() gin.HandlerFunc{
	return func (c *gin.Context){
		...
		c.Join(http.StatusOK,allOrderItems)
	}
}
...
func ItemsByOrder(id string) (OrderItems [] primitive.M, err error) {
	...
	projectStage :=bson.D{
		...
		groupStage :=bson.D{{"$group",bson.D{{"_id",bson.D{{"order_id","$order_id"},{"table_id","$tabke_id"},{"table_number","$table_number"}}},{"payment_due",bson.D{{"$sum","$amount"}}},{"total_count",bson.D{{"$sum",1}}},{"order_items",bson.D{{true}}}}}}
...
func GetOrderItem() gin.HandlerFunc{
	return func (c *gin.Context){
		...
		defer cancel()
		if err!=nil {
			c.JSON(http.StatusInternalServerError,gin.H{"error":"error occured while listin order item"})
			return
		}
		c.JSON(http.StatsuOk, orderItem)
...
func UpdateOrderItem() gin.HandlerFunc{
	return func (c *gin.Context){
		...
		if orderItem.Unit_price !=nil{
			updateObj = append(updateObj,bson.E{"unit_prive",*&orderItem.Unit_price})
		}
